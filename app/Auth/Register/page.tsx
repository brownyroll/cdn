...
export default function SigninPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('/api/auth', { type: 'register', username, password , fullname});
        ...
        <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
          ...
          <form onSubmit={handleSubmit}>
            <Input
              ...
              className="w-full m-5 -ml-10 pl-10 pr-3 py-2 rounded-lg outline-none focus:border-indigo-500"
            />
            <Input
              type="text"
              label="Fullname"
            ...
            <div className="flex -mx-3">
              <div className="w-full px-3 mb-5">
                <button type="submit" className="block m-5 w-full max-w-xs mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-3 py-3 font-semibold">REGISTER NOW</button>
              </div>
            </div>
          </form>
        </div>
